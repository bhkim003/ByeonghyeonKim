...
'''
여기에 넣어라 # BH code 부분 수정한거임
이게 duration으로 dvs gesture 만들면 50장까지만 만드게 해준다.
/home/bhkim003/anaconda3/envs/aedat2/lib/python3.8/site-packages/spikingjelly/datasets/__init__.py
...
def integrate_events_by_fixed_duration(events: Dict, duration: int, H: int, W: int) -> np.ndarray:
    ...
    while True:
        ...
        left = right
        full_time_step = full_time_step + 1# BH code
        # if right == N:
        #     return np.concatenate(frames)
        if right == N or full_time_step == 50:  # BH code
            return np.concatenate(frames) # BH code
